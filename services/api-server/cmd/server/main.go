...
func main() {
	...
	diHandlers := &router.Handlers{
		...
		Pull:         handlers.NewPullRequestHandler(pullSvc, repoSvc, authzSvc),
		Commit:       handlers.NewCommitHandler(gitSvc, repoSvc, authzSvc),
		File:         handlers.NewFileHandler(gitSvc, repoSvc, authzSvc),
		Contributor:  handlers.NewContributorHandler(repoSvc, contributorRepo, userRepo, authzSvc),
